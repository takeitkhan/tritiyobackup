...
class Payments_model extends Common_model
{
    ...
    public function get_ledger_names($ledType = 1)
    {
        $this->db->select('*, IF(LedgerTypeId = 1, \'Income\', \'Expense\') AS LedgerType')->from($this->_ledgernames)
          	->where('LedgerTypeId', $ledType)
            ->order_by('LedgerName');
        $result = $this->db->get();
        $return = array();
        if ($result->num_rows() > 0) {
