...
class Results extends MY_Controller
{
    ...
    public function index()
    {
        ...
        if(isset($_GET['export']) == 'yes' && count($this->data['allresults']) > 0){
            $this->generateresultsexport($this->data['allresults']);
        }
    ...
    public function generateresultsexport($results){
        //owndebugger($results);
        //exit();
         ...
         fputcsv($out,['Result Id','Student ID','Roll','Name','Written','MCQ','Practical','IsAbsent'],"\t");
         foreach ($results as $data)
         {
             fputcsv($out, [$data['ResultId'],$data['StudentId'],$data['ClassRoll'],tableSingleColumn('users', 'full_name_en', ['id'=> $data['StudentId']]),$data['Written'],$data['MCQ'],$data['Practical'],$data['IsAbsent']],"\t");
         }
         fclose($out);
    ...
    public function resultImport(){
        if(isset($_POST['excelfiles'])){
        $content = file_get_contents($_FILES['excelfile']['tmp_name']);
        $delimiter = $this->detectDelimiter($_FILES['excelfile']['tmp_name']);
        ...
        foreach ($data as $row) {
            ...
            $sqldata = ['Written' => $row[4],'MCQ'     => $row[5],'Practical' => $row[6],'IsAbsent'  => $row[7]];
            // owndebugger($row);
          // owndebugger($sqldata);
           
           $this->db->where('ResultId',$row[0]);
           $this->db->update($this->common_model->_results,$sqldata);
    ...
    **************************************/
    ...
    public function if_result_exists($eid, $cid, $sid, $subid, $sgid, $year)
    {
        ...
        $subjectRule = array(
                          ...
                          's_pass' => $subject_rule->separate_pass,
                          'noeffect' => $subject_rule->noeffect,
        ...
        //owndebugger($subjectRule_json);
            ...
            $this->db->set('Subject_rule', $subjectRule_json);
            $this->db->where($where2);
            $this->db->update($this->common_model->_results);
    ...
    public function resultexport(){
        ...
        $this->data['userid'] = $this->data['userInformation']->id;
        $this->data['title'] = "Get Result Report";
        ...
        $this->load->view('layouts/header', $this->data);
        $this->load->view('reports/result_export', $this->data);
        $this->load->view('layouts/footer', $this->data);
    }
    public function result_pdf() {
        ...
        $this->data['eid'] = (isset($_GET['examid']) ? $_GET['examid'] : '0');
        $this->data['cid'] = (isset($_GET['classs']) ? $_GET['classs'] : '0');
        $this->data['sid'] = (isset($_GET['section']) ? $_GET['section'] : '0');
        $this->data['subid'] = (isset($_GET['subject']) ? $_GET['subject'] : '0');
        $this->data['sgid'] = (isset($_GET['groupp']) ? $_GET['groupp'] : '0');
        $this->data['year'] = (isset($_GET['yearid']) ? $_GET['yearid'] : '0');
        $this->data['msg'] = (isset($_GET['msg']) ? $_GET['msg'] : 'Select Appropriate Exam, Class, Section, Subject and Group to Generate the Result.');
        ...
        if (!empty($this->data['ui'])) {
            $rolls = explode(',',$this->data['ui'] );
            foreach($rolls as $roal){
                $result = $this->db->where('ClassRoll', $roal)
                ->where('Classes', $this->data['cid'])
                ->where('Sections', $this->data['sid'])
                ->where('AddedYear', $this->data['yearid'])
                ->where('Exams', $this->data['examid'])
                ->get('results')->result_array();
                
                //return var_dump($result);
                
                $studen_id = $result[0]['StudentId'];
                $biodata = $this->common_model->get_user_single_informations_by_user_id($studen_id);
                // echo $this->db->last_query();
                //  owndebugger($biodata);
                // 
                $result_info[] = array(
        ...
        //owndebugger($result_info);
        //die();
        
        
        //return owndebugger($result_info);
        
        $this->load->library('pdf');
        $this->load->model('pdf_model');
        $html_content = $this->pdf_model->result_export($result_info);
