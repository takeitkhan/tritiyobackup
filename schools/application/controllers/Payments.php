...
class Payments extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['ledgernames'] = $this->get_all_ledger_names(1);
    ...
    public function new_credit_payment()
    {
        ...
        $this->data['loggedinuserinformation'] = $this->basic_information_of_mine($this->data['userInformation']->id);
        $this->data['userid'] = $this->data['userInformation']->id;
        $this->data['title'] = "Add New Income";

        $this->data['ledgernames'] = $this->get_all_ledger_names(1);
        $this->data['pay_method'] = $this->get_payment_methods();
      	$this->data['payment_type'] = 'Income';
        $this->load->view('layouts/header', $this->data);
        $this->load->view('payment/addpayment', $this->data);
        $this->load->view('layouts/footer', $this->data);
    }
  	...
  	public function new_debit_payment()
    {
        ...
        $this->data['userid'] = $this->data['userInformation']->id;
        $this->data['title'] = "Add New Expense";

        $this->data['ledgernames'] = $this->get_all_ledger_names(2);
        $this->data['pay_method'] = $this->get_payment_methods();
      	$this->data['payment_type'] = 'Expense';
        $this->load->view('layouts/header', $this->data);
        $this->load->view('payment/addpayment', $this->data);
        $this->load->view('layouts/footer', $this->data);
    ...
    public function get_all_ledger_names($type)
    {
        $row = $this->payments_model->get_ledger_names($type);
        return $row;
    }
    ...
    public function get_payment_methods()
    {
      ...
      if ($this->ion_auth->logged_in()) {
        ...
        $this->data['title'] = "Reports";
      	...
      	$rows = $this->db->query('SELECT * FROM acc_payments ORDER BY PaymentId DESC LIMIT 0, 30 ');      	
      	$this->data['results'] = $rows->result_object();
        //$this->data['ledgers'] = $this->get_ledger_names_by_typeid(1);
        $this->data['pay_method'] = $this->get_payment_methods();

        $this->load->view('layouts/header', $this->data);
        $this->load->view('payment/reports', $this->data);
        $this->load->view('layouts/footer', $this->data);                 
    }
}
