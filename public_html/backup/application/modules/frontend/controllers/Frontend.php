...
//        $this->output->enable_profiler(TRUE);
    ...
    public function index() {
        $this->data['title'] = 'Tritiyo Limited | valid reason, dynamic solution';
        $this->data['SeoTitle'] = 'Tritiyo Limited | valid reason, dynamic solution';
    ...
    public function blog() {
        $data['title'] = 'Web design, Web Development, Digital Marketing, Android app development blog';
        $data['SeoTitle'] = 'Web design, Web Development, Digital Marketing, Android app development blog';
        $data['MetaKeyword'] = 'best web design company in Bangladesh, web development, digital marketing, android app development, mobile app development';
        $data['MetaDescription'] = 'Web design, Web development, Digital marketing, Android app development blog';
        $offset = ($this->input->get('per_page')) ? $this->input->get('per_page') : 0;
        $per_page_limit = 16;
        $url = base_url('blog');
        
        $data['posts'] = $this->db->select('*')->from('posts')->where('Category', 180)->order_by('PostId', 'desc')->limit($per_page_limit)->offset($offset)->get()->result();
                    
        $data['recent_posts'] = $this->db->select('*')->from('posts')->where('Category', 180)->order_by('PostId', 'desc')->limit(5)->get()->result();            
        $total_rows = $this->db->select('*')->from('posts')->where('Category', 180)->get()->num_rows();
    ...
    public function blog_single($slug) {
        ...
        $post = $this->db->select('*')->from('posts')->where('PostRoute', $slug)->get()->result();
        $this->data['single_post'] = $post[0];
        $this->data['recent_posts'] = $this->db->select('*')
                    ->from('posts')
                    ->where('Category', 180)
        ...
        $this->data['title'] = $this->data['single_post']->Title;
        $this->data['SeoTitle'] = $this->data['single_post']->Title;
        $this->data['MetaKeyword'] = 'best web design company in Bangladesh, web development, digital marketing, android app development, mobile app development';
        $this->data['MetaDescription'] = $this->data['single_post']->Title;
        
        
        //owndebugger($this->data);
        //die();
        $this->load->view('header', $this->data);
        $this->load->view('blog_single', $this->data);
        $this->load->view('footer', $this->data);
    }
