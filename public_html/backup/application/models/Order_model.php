...
//	RENAME COLUMN `code1` TO  `code`");
		...
		/*$x=$this->db->query("ALTER TABLE ecommerce_order_master MODIFY order_id int(11) null");
		...
		*/
		        $html = '';
        $res = mysqli_query('DESCRIBE ecommerce_order_details');
        while($row = mysqli_fetch_array($res)) {
            echo "{$row['Field']} - {$row['Type']}\n";
        }

        $qs = $this->db->query("SHOW COLUMNS FROM ecommerce_order_details")->result();
        foreach($qs as $q) {
		...
		//$x = $this->db->query("SELECT * FROM ecommerce_order_master order_by id 'DESC'");
        //return $x->result_array();
		
        die();

/*$x=$this->db->query("SELECT * FROM `ecommerce_order_details` order by master_id desc");
		$y = $x->result_array();
		owndebugger($y);*/
//                $x = $this->db->query("ALTER TABLE `products`
