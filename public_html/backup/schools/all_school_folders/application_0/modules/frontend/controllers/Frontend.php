...
class Frontend extends MX_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model(array("common_model", "result_model", "profile_model", "settings_model", "panel_model", "payments_model", "frontend_model", "shortcodes/photogallery"));
    ...
    public function get_notice($notice_id)
    {
        ...
        $this->data['settings'] = $this->get_settings();
        $this->blocks();
        $this->data['notice'] = $this->db->get_where('posts', array('PostId =' => $notice_id))->result();
        $this->load->view('template/notice', $this->data);
    }
    ...
    public function all_news(){
        ...
        $this->data['title'] = 'সকল সংবাদ';
        //$this->data['notices'] = $this->get_all_posts_by_cat_id(177);
        //$this->data['downloads'] = $this->get_all_posts_by_cat_id(178);
        $this->load->view('template/all-news', $this->data);
    }
    public function all_notice(){
        $this->data['settings'] = $this->get_settings();
        $this->blocks();
        $this->data['title'] = 'সকল নোটিশ';
        $this->load->view('template/all-notice', $this->data);
    }

    public function all_download(){
        $this->data['settings'] = $this->get_settings();
        $this->blocks();
        $this->data['title'] = 'সকল ডাউনলোড';
        $this->load->view('template/all-download', $this->data);
    }
    ...
    {
        $result  = $this->db->get_where('posts', array( 'PostId' => $id))->row();
        //owndebugger($result); die();

        $this->load->helper('download');

        $path = base_url().'uploads/posts/'.$result->MediaFileName;
        $data =  file_get_contents($path);
        force_download($result->MediaFileName, $data);
    }
...
//        $this->results = $this->payments_model->insertPaymentEntries($data1);
    ...
    public function admission_information() {
        ...
        $userid = $this->input->post('userid');
        $dateofbirth = datetoint($this->input->post('dob'));
...
//                'Address' => $this->input->post('address'),
            ...
            $data1 = array(
               ...
               'name_institute' => $this->input->post('name_institute'),
               'institute_address' => $this->input->post('institute_address'),
               'guardian_profession' => $this->input->post('guardian_profession'),
               'annual_income' => $this->input->post('annual_income'),
               'absence_father_guardian' => $this->input->post('absence_father_guardian'),
               'relation' => $this->input->post('relation'),
               'study_loc' => $this->input->post('study_loc'),
...
//            owndebugger($this->status);
    ...
    public function admission_information_new() {
        $dateofbirth = datetoint($this->input->post('dob'));
        ...
        $data = array(
            'random_code' => $this->input->post('randomcode'),
            'json_data' => json_encode($this->input->post()),
            'userphoto' => $profilephoto['file_name'],
            'created_on' => $this->input->post('created_on')            
        );
        $this->results = $this->profile_model->insertAdmissionData($data);
        
        if (!empty($this->results)) {            
            redirect('admission_thankyou?getrandomcode=' . $this->input->post('randomcode'), 'refresh');    
        } else {
            $this->status['status'] = 0;
            $this->status['msg'] = 'Oh snap! Change a few things up and try submitting again.';
        }
    ...
    public function admission_thankyou() {
        $this->data['settings'] = $this->get_settings();
        $this->blocks();

        $this->data['random'] = (!empty($this->input->get('getrandomcode')) ? $this->input->get('getrandomcode') : '');
        
        $this->data['form_data'] = $this->profile_model->getAdmissionData($this->data['random']);
        
        $this->load->view('template/header', $this->data);
        $this->load->view('admission/thank_you', $this->data);
        ...
        foreach ($menus as $menu) {
            ...
            if (!empty($menu->Childs)) {
                ...
                foreach ($childs as $child) {
                    $menu = explode(';', $child);
                     if(trim($menu[1]) == 'StudentsList'){
                        $html .= '<li><a href="' . base_url() . 'page/students">' . $menu[2] . '</a></li>';
                    }else{
    ...
    public function page_students(){
        ...
        $this->blocks();
        $this->data['latestnews'] = $this->get_all_posts_by_cat_id(176);
        $this->data['title'] = 'সকল ছাত্র ছাত্রী';
        ...
        $search = array(
            ...
            'Class' => $this->data['cid'],
            'GroupId' => $this->data['sgid'],
            'limit' => $limit,
            'offset' => $offset,
            'prm' => $prm
        );
        $this->data['students'] = $this->common_model->get_student_list_front($search);
        $this->load->library('pagination');
        $config['total_rows'] = $this->common_model->count_student_list_front($search);
        ...
        $config['base_url'] = base_url('page/students');
        $this->pagination->initialize($config);
        $this->data['paging'] = $this->pagination->create_links();
        //$this->load->view('layouts/header', $this->data);
        $this->load->view('template/students', $this->data);
