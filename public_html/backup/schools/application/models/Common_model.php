...
class Common_model extends CI_Model
{
    ...
    public $_studentSubjectsInfo = 'u_std_subjects_info';
    ...
    public $_admissiontable = 'admission_table';
    public $_result_rule_group = 'result_rule_group';
    public $_rule_group_subjects = 'rule_group_subjects';
    ...
    function get_settings($id)
    {
        ...
        function get_sub_setting()
    {
        $this->db->from($this->_initialSettings)
            ->where('SettingsCategory', 4)
...
//(SELECT FROM_UNIXTIME(DateOfBirth, \'%Y-%m-%d\') FROM u_basicinfos WHERE UserId = user_id) AS dob,
    function get_user_single_informations_by_user_id($userid)
    {
        ...
        FROM users_groups WHERE user_id = "'. $userid .'"');
        $sql = $this->db->get();
        if ($sql->num_rows() > 0) {
            $data = $sql->result();
            return $data;
    ...
    public function all_totals()
    {
        $sql = 'SELECT * FROM
                    ...
                    (SELECT COUNT(*) AS totalstudents FROM users_groups LEFT JOIN u_basicinfos ON users_groups.user_id = u_basicinfos.UserId WHERE group_id = 4 AND u_basicinfos.EnrollmentStatus = 49) AS ts,
    ...
    function get_student_list_front($search){
        $this->db->select('*');
        ...
        $this->db->order_by('ClassRoll');
        $query = $this->db->get('u_std_information', $search['limit'], $search['offset']);
        
        $row = $query->result_array();
        return $row;
    }
    function count_student_list_front($search){
        $this->db->select('*');
        $this->db->where('isActive', 1);
        if($search['Class']) {
            $this->db->where('Class', $search['Class']);
    ...
    public function document_report(array $search){
      ...
      if(@$search_final['section'] && @$search_final['class']) {
        $this->db->select('*');
        $this->db->where('isActive', 1);

        if(@$search_final['student']) {
            $this->db->where('UserId', $search_final['student']);
        }
        if(@$search_final['section']) {
            $this->db->where('Section', $search_final['section']);
        }
        if(@$search_final['class']) {
            $this->db->where('Class', $search_final['class']);
        }

        $this->db->order_by('ClassRoll', ' ASC');
        $query = $this->db->get('u_std_information', $search_final['limit'], $search_final['offset']);
        $row = $query->result();
        return $row;
      }else {
        return false;
  ...
  public function document_report_total(array $search){
    ...
    if(@$search_final['section'] && @$search_final['class']) {
      ...
      $this->db->order_by('ClassRoll', ' ASC');
      $query = $this->db->get('u_std_information');
      $row = $query->num_rows();
      return $row;
    }else {
      return false;
