...
class StudentsList extends Common_model {
		...
		if (! empty ( $rows )) {
			...
			if (! empty ( $_GET ['Class'] )) {
				...
				$str .= '<ul class="list-group">';
                                ...
                                $fileurl = base_url() . 'uploads/pp/' . $biodata['userphoto'];
                                $blankavatar = base_url() . 'uploads/pp/blankavatar.jpg';
                                $img = __checkimgavailable($fileurl, $blankavatar);
                                
				$str .= '<li>' . (isset($biodata['userphoto']) ? ' Photo:<br/> <img src="' . $img . '" width="150" />' : '') . '</li>';
				$str .= '<li>' . (isset($biodata['genderen']) ? ' Gender: ' . $biodata['genderen'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['dob']) ? ' Date of Birth: ' . $biodata['dob'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['bloodgroupen']) ? ' Blood Group: ' . $biodata['bloodgroupen'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['phone']) ? ' Phone: ' . $biodata['phone'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['religionen']) ? ' Religion: ' . $biodata['religionen'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['upazilaen']) ? ' Upazilla: ' . $biodata['upazilaen'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['districten']) ? ' District: ' . $biodata['districten'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['country']) ? ' Country: ' . $biodata['country'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['maritalstatusen']) ? ' Marital Status: ' . $biodata['maritalstatusen'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['enrollmentstatus']) ? ' Enrollment Status: ' . $biodata['enrollmentstatus'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['groupname']) ? ' Group Name: ' . $biodata['groupname'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['enstdinfo']) ? ' Student Information:<br/> ' . $biodata['enstdinfo'] : '') . '</li>';
				$str .= '<li>' . (isset($biodata['educationhistory']) ? ' Education History:<br/> ' . $biodata['educationhistory'] : '') . '</li>';
				$str .= '</ul>';
				$str .= '</div>';
				$str .= '</div>';
				$str .= '</div>';
	...
	public function getAllEnteredClasses() {
		$this->db->select ( 'Class,
            ...
            (SELECT SettingsDescription FROM initial_settings_info WHERE SettingsId = Class) AS ClassNameBn' )->from ( $this->_studentInformation )->join('u_basicinfos', 'u_basicinfos.UserId = u_std_information.UserId', 'left')->where('EnrollmentStatus', 49)->group_by ( 'Class' );
