    ...
    **************************************/
        ...
        if($posts){
            ...
            if($this->common_model->isRecordsExists('initial_settings_info',['SettingsCategory'=> 4 ,'SettingsName'=>$posts['SettingsName'], 'class_level'=>$posts['class_level']]))
            {
               ...
               redirect('subjectssettings');
                ...
                $mergeable = (!empty($posts['mergeable']))? $posts['mergeable'] : '';
                $noeffect = (!empty($posts['noeffect']))? $posts['noeffect'] : '';
                $data = [
                    'SettingsCategory'=> 4 ,
    ...
    public function edit_subject($id)
    {
        ...
        if(@$posts['SettingsId']){
            ...
            $mergeable = (!empty($posts['mergeable']))? $posts['mergeable'] : null;
            $noeffect = (!empty($posts['noeffect']))? $posts['noeffect'] : null;
