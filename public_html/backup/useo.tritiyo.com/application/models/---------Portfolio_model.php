...
class Portfolio_model extends CI_Model {
    ...
    public function insert_technology($data) {
        $this->db->insert('portfolio_technology', $data);
        return $this->db->insert_id();
    }
    public function update_technology($data, $id){
        if($this->db->where('id', $id)->update('portfolio_technology', $data)){
            return true;
        }
    }
    public function get_technologys() {
        if (!isset($this->data[__METHOD__])) {
            $this->data[__METHOD__] = $this->db->select('*')
    ...
    public function thisTechnologyInfo($id){
        $where = array('id' => $id);
        return $this->db->get_where('portfolio_technology',$where)->result();
    }
    public function delete_technology($id){
        $del = $this->db->delete('portfolio_technology', array('id' => $id));
        return ($del) ? 1 : 0;
    }
