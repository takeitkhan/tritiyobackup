                                ...
                                <table style="width: 900px;table-layout:fixed; font-size: 12px; font-family: 'Trebuchet MS', sans-serif;">
                                    ...
                                    if(!empty($result)) {
                                      ...
                                      $bangla = [];
                                        ...
                                        foreach($result as $key => $value) {
                                            ...
                                            if (in_array(@$value['subject_id'], $math_id)) {
                                                $math_subject_id = $value['subject_id'];
                                                $math_written_pass_mark = trim($value['written_pass_mark']);
                                                $math_mcq_pass_mark = trim($value['mcq_pass_mark']);
                                                $math_written = trim($value['written']);
                                                $math_mcq = trim($value['mcq']);

                                                if (($math_mcq < $math_mcq_pass_mark) || ($math_written < $math_written_pass_mark)) {
                                                    $math_total = "<span style='color:red'>" . ($value['written'] + $value['mcq']) . "</span>";
                                                    $math_grade = "<span style='color:red'>F</span>";
                                                    $math_gpa = 0;
                                                } else {
                                                    $math_total = ($value['written'] + $value['mcq']);
                                                    $math_grade = makeGradeHundred($math_total);
                                                    $math_gpa = makeGpaHundred($math_total);
                                                }
                                            }


                                            $bn1mcq = @globalyFailCheck(@$bangla_1st_mcq, $bangla_1st_subject_id, 'MCQ');
                                            $bn1cq = @globalyFailCheck(@$bangla_1st_written, $bangla_1st_subject_id, 'CQ');
                                            $bn2mcq = @globalyFailCheck(@$bangla_2nd_mcq, $bangla_2nd_subject_id, 'MCQ');
                                            $bn2cq = @globalyFailCheck(@$bangla_2nd_written, $bangla_2nd_subject_id, 'CQ');
                                    ...
                                    <tr>
                                        ...
                                        <td style="text-align: left;border: 1px solid #DDD; border-collapse: collapse; padding: 5px;">
                                          ...
                                          <?php $bn1mcq = @globalyFailCheck(@$value['Written'], @$value['Subjects'], 'CQ') ;

                                          if ($bn1mcq['status'] == 'pass') {
                                            echo @$value['Written'];
                                          } else {
                                              echo "<span style='color:red'>".@$value['Written']."</span>";
                                          }
                                          ?>
                                        ...
                                        <td style="text-align: left;border: 1px solid #DDD; border-collapse: collapse; padding: 5px;">
                                              ...
                                              <?php
                                                if($value['Subjects'] == 370) {echo $Bangla_gpa;
                                                }elseif($value['Subjects'] == 369){
                                                  echo " ";
                                                }else {echo @$gpa; }
                                                ?>
                                        </td>
