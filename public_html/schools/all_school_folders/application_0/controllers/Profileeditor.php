...
class ProfileEditor extends MY_Controller
{
    ...
    public function users_information()
    {
        ...
        $this->data['personal_info'] = $this->personal_information_of_mine($this->uri->segment(2));
        $this->data['status_info'] = $this->db->get_where('u_std_information', array('UserId' => $this->uri->segment(2)))->row();
        //owndebugger($this->data['status_info']);
    ...
    public function modify_basic_information()
    {
        ...
        $this->data['gender'] = $this->gender_get(7);
        $this->data['enrollment'] = $this->common_model->get_settings(10);
        $this->data['personal_info'] = $this->personal_information_of_mine($this->data['userid']);
        if ($this->data['loggedinusergroupid'] != 1) {
            redirect('dashboard');
        } else {
            // owndebugger($this->input->post('stu_status'));
            // die();
            ...
            if($this->input->post('stu_status')){
                if($this->input->post('stu_status') == 1){
                    $this->db->update('u_std_information', array('isActive' => '1'), array('UserId' => $uri2));
                }else{
                    $this->db->update('u_std_information', array('isActive' => '0'), array('UserId' => $uri2));
                }
            }else{
                    $this->db->update('u_std_information', array('isActive' => '0'), array('UserId' => $uri2));
                }

            if($this->input->post('enrollmentstatus')){
                ...
                if (isset($this->data['userid'])) {
                    ...
                    //owndebugger($stu_data);

                    if ($this->input->post('infosid') == 'none') {
                        $this->results = $this->profile_model->insertPersonalInformation($stu_data);
                    } else {
                        $where = array('InfosId' => $this->input->post('infosid'));
                        $this->results = $this->profile_model->updatePersonalInformation($stu_data, $where);
                    }
                }
            }
...
//    }
    ...
    public function student_personal_information()
    {
        ...
        if ($this->data['loggedinusergroupid'] != 1) {
           ...
           //$user_type = $this->ion_auth->get_users_groups(201634286)->row();
            ...
            $groupinfos = $this->get_group_name($this->data['userid']);
            $this->data['groupname'] = $groupinfos->name;
            $this->data['selected_subject'] = $this->db->get_where('u_std_subjects_info',['UserId'=>$this->data['userid']])->row();
            // dd($this->data['selected_subject'] );
    ...
    public function get_subjects_by_group($value='')
    {
        $groupid = $this->input->post('str');
       ...
       // owndebugger($subjects);
        $religion = array(309,310,311);
        $html = '';
        foreach ($mandatory_subjects as $subject) {
            $selected = (in_array($subject->SettingsId ,$religion))? "": "selected";
            $html .= "<option ".$selected." value='". $subject->SettingsId ."'> $subject->SettingsDescription </option>";
        }

        $html2 = '';
        foreach ($optional_subjects as $subject) {
            $html2 .= "<option value='". $subject->SettingsId ."'> $subject->SettingsDescription </option>";
        }

        echo jsonEncode(['html'=> $html,'html2'=> $html2]);
    }
    ...
    public function modify_student_personal_information()
    {
        ...
        if ($this->data['loggedinusergroupid'] != 1) {
            ...
            /*$common_subjects = $this->input->post('common_subject');
            if($common_subjects){
                $selected_common_subject = '';
                foreach ($common_subjects as $common_subject) {
                     $selected_common_subject .= $common_subject .',';
                }
            }*/


            $mandatory_subjects = $this->input->post('mandatory_subject');
            if($mandatory_subjects){
                $selected_mandatory_subject = '';
                foreach ($mandatory_subjects as $mandatory_subject) {
                     $selected_mandatory_subject .= $mandatory_subject .',';
                }
            }
            ...
            $data3 = array(
                ...
                'mandatory_subjects' =>($selected_mandatory_subject)? rtrim($selected_mandatory_subject,','):'',
            ...
            if ($this->input->post('studentpersonalinfoid') == 'none' || $this->input->post('teacherstaffinfoid') == 'none') {
                if ($groupcheck == 4) {
                    ...
                    if($data3){
                        $this->results = $this->profile_model->insertStudentsubjectInformation($data3);
                    }
                ...
                if ($groupcheck == 4) {
                    if($this->input->post('promote') == 2) {
                        /*promote_previous_data*/
                        $promote_previous_data =[
                            'UserId' => ((isset($uri2) ? $uri2 : $this->data['userid'])),
                            'Class' => $this->input->post('old_class'),
                            'ClassRoll' => $this->input->post('old_classroll'),
                            'Section' => $this->input->post('old_section'),
                            'GroupId' => $this->input->post('old_group'),
                            'Department' => $this->input->post('old_department'),
                            'StudyYear' => $this->input->post('old_year')
                        ];
                        unset($data['isActive']);
                        $data2 = array(
                            'StudyYear' => date('Y')
                        );
                        $datamerged = array_merge($promote_previous_data, $data2);
                        $this->results = $this->profile_model->insertStudentPromotion($datamerged);
                        // $this->results = $this->profile_model->insertStudentPromotion($promote_previous_data);
                    }
                    ...
                    if($data3){
                        if($this->common_model->isRecordsExists($this->common_model->_studentSubjectsInfo,['UserId'=> $data3['UserId']])){
                            $where2 = array('UserId' => $data3['UserId']);
                            $this->results = $this->profile_model->updateStudentSubjectInformation($data3, $where2);
                        }else{
                            $this->results = $this->profile_model->insertStudentsubjectInformation($data3);
                        }
