...
function basic_information_of_mine_helper($id)
{
           get_instance()->load->model("profile_model");
    return get_instance()->profile_model->getUsersInformation($id);
}
...
if (!function_exists('put_headers')) {
    ...
    function put_headers()
    {
        ...
        foreach ($header_css AS $item) {
            $str_css[] = $item;
            $str .= '<link rel="stylesheet" href="http://www.tritiyo.com/schools/' . $item . '" type="text/css" />' . "\n";
        }
        return $str;
    }
...
if (!function_exists('put_footer')) {
    ...
    function put_footer()
    {
        ...
        foreach ($footer_js AS $item) {
            $str .= '<script  type="text/javascript"  src="http://www.tritiyo.com/schools/' . $item . '"></script>' . "\n";
        }
...
/**
 * @package: NewPortal
 * @helper::put_local_footer().
 * @Author: Musa
 */
if (!function_exists('put_local_footer')) {

    function put_local_footer()
    {
        $str = '';
        $ci = &get_instance();
        $footer_js = $ci->config->item('footer_js');
...
function sendSms($message, $receiver, $stdid) {
   $CI = get_instance();
  // $CI->load->model('sms_model');
  // $CI->sms_model->send_singel_sms($receiver, $message, $stdid);

      $mysetting =  $CI->db->select('*')->where('id', 1)->get(tbl_sms_setting)->row();
  ...
  if($limit_sms > 0){
      ...
      $param="user=$user&pass=$pass&sms[0][0]= $receiver &sms[0][1]=".urlencode($message)."&sms[0][2]=123456789&sid=$sid";
      $crl = curl_init();
      ...
      curl_setopt($crl, CURLOPT_CONNECTTIMEOUT, 30); //old function
      curl_setopt($crl, CURLOPT_TIMEOUT, 30); //old function
      curl_setopt($crl,CURLOPT_POST,1);
      curl_setopt($crl,CURLOPT_POSTFIELDS,$param);
      curl_exec($crl);
      curl_getinfo($crl, CURLINFO_HTTP_CODE); //old function
      if (curl_error($crl)) {
      ...
      curl_close($crl);
        if (isset($error_msg)) {
          ...
          $data = [
              'send_number'=> $receiver,
              'message '=> @$message,
              'user_id '=> @$stdid,
              'lot_id '=> $mysetting->current_lot
          ];

          $u_data = ['sms_limit'=> $limit_sms - 1,];
          $CI->db->update(tbl_sms_setting, $u_data, "id = 1");
          $CI->db->insert('sms_history', $data);
        }
    }
  }
}
 ...
 * @return bool|string
 */
function passCheck($score, $min, $max) {
 ...
 * @return bool|string
 */
function globalSubjectFailCheck($score, $subject_id, $type) {
    $CI = get_instance();
    $html = '';
    $pass_marks= @$CI->db->get_where('initial_settings_info', ['SettingsId' => $subject_id])->row();
    ...
    if($type == 'CQ'){
        if(($score >= $cq_pass_mark) && ($score <= $cq_max_mark)){
            $html = "<span>$score</span>";
        }else{
            $html = "<span style='color:red'>$score</span>";
        }
    }
    elseif($type == 'MCQ'){
        if(($score >= $mcq_pass_mark) && ($score <= $mcq_max_mark)){
            $html = "<span>$score</span>";
        }else{
            $html = "<span style='color:red'>$score</span>";
        }
        ...
        if($score >= $p_pass_mark && $score <= $p_max_mark){
            $html = "<span>$score</span>";
        }else{
            $html = "<span style='color:red'>$score</span>";
        }
...
function globalSubjectFailCheck2($score, $subject_id, $subject_rule, $optional, $type) {
    $CI = get_instance();
    $html = '';


    //$pass_marks= @$CI->db->get_where('initial_settings_info', ['SettingsId' => $subject_id])->row();
    $pass_marks = json_decode($subject_rule);
    $cq_max_mark = $pass_marks->cq ;
    $mcq_max_mark = $pass_marks->mcq ;
    $p_max_mark = $pass_marks->practical ;
 ...
 * @return bool|string
 */
function globalyFailCheck($score, $subject_id, $type) {
    $CI = get_instance();
    $html = '';
    $status = '';
    $pass_marks= @$CI->db->get_where('initial_settings_info', ['SettingsId' => $subject_id])->row();
...
// =IF(AS7>=40,"A+",IF(AS7>=35,"A",IF(AS7>=30,"A-",IF(AS7>=25,"B",IF(AS7>=20,"C",IF(AS7>=17,"D","F"))))))
// =IF(BC6>=20,"A+",IF(BC6>=17,"A",IF(BC6>=15,"A-",IF(BC6>=12,"B",IF(BC6>=10,"C",IF(BC6>=8,"D","F"))))))

function makeGradeTwentyFive($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=20)
        return 'A+';
    if($score>=17)
        return 'A';
...
function makeGpaTwentyFive($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=20)
        return 5;
    if($score>=17)
       return 4;
...
function makeGradeFifty($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=40)
        return 'A+';
    if($score>=35)
        return 'A';
...
function makeGpaFifty($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=40)
        return 5;
    if($score>=35)
       return 4;
...
function makeGradeHundred($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=80)
        return 'A+';
    if($score>=70)
        return 'A';
...
function makeGpaHundred($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=80)
        return 5;
    if($score>=70)
        return 4;
...
function makeGradeHundredFifty($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=120)
        return 'A+';
    if($score>=105)
        return 'A';
...
function makeGpaHundredFifty($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=120)
        return 5;
    if($score>=105)
        return 4;
...
function makeGradeTwoHundred($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=160)
        return 'A+';
    if($score>=140)
        return 'A';
...
function makeGpaTwoHundred($score) { //just a rewrite of your own code, exactly the same purpose
    if($score>=160)
        return 5;
    if($score>=140)
        return 4;
...
function global_results_single($user_id = '',$subject = '', $exam = '', $class = '', $section = '', $year = '', $return = '') {
    $CI = get_instance();
    $html = '';
    $cgpa = '';
    $grade = '';
    $all = '';
    $rs = $CI->reports_model->getSubjectResultByUserByYear($user_id, $subject, $class, $section, $exam, $year);
// owndebugger($rs);
    //getResultByUserByYear($id, $class, $section, $examid, $year)
    if(!empty($rs)){

    foreach($rs as $key => $value) {
        $total = array($value['Written'], $value['MCQ'], $value['Practical']);
        $totalSum = array_sum($total);
        // echo $totalSum;
        if($return == 'grade') {
            echo  $grade = makeGrade($totalSum);
        }else if($return == 'cgpa') {
             echo $gpa = makeGpa($totalSum);
        }
        ...
        return 'Not Found Any Result in Database';
    }
}
...
function global_results($user_id = '', $exam = '', $class = '', $section = '', $year = '', $return = '') {
    $CI = get_instance();
    $html = '';
    $cgpa = '';
    $grade = '';
    $all = '';
    $rs = $CI->reports_model->getResultByUserByYear($user_id, $class, $section, $exam, $year);
   //owndebugger($rs);
    //getResultByUserByYear($id, $class, $section, $examid, $year)
    if(!empty($rs)){
    ...
    foreach($rs as $key => $value) {
        ...
        $totalSum = array_sum($total);
        // echo $totalSum;
        $grade = gradeGenerating($value['FullMarks'],$value['PassMarks'],$totalSum);
        $gpa = gpaGenerating($value['FullMarks'],$value['PassMarks'],$totalSum);
        $gradesum += $gpa;
        $grades[] = $grade;
    ...
    $avgcgpa = sprintf("%.2f", $gradesum/count($rs));
...
function paging($total_rows, $per_page_limit, $url){
    globalInstance()->load->library('pagination');
    $config['total_rows'] = $total_rows;
    $config['per_page'] = $per_page_limit;
    $config['enable_query_strings'] = TRUE;
...
function GradeGenerate($intotal, $getMark){
     $my_grade = '';
    if($intotal == 200){
...
function GpaGenerate($intotal, $getMark){
    $my_gpa = '';
    if($intotal == 200){
...
/*New Function to generate GPA/GRADE*/
function gpaGenerating($fullmarks,$passmarks,$totalmarks){
    if($totalmarks < $passmarks){
        return '0';
    }
    
    return GpaGenerate($fullmarks,$totalmarks);
}

function gradeGenerating($fullmarks,$passmarks,$totalmarks){
    if($totalmarks < $passmarks){
        return '0';
    }
    
    return GradeGenerate($fullmarks,$totalmarks);
}
